...
while revancha:
    ...
    ocean = utils.ocean_dimension()
    fleet = utils.fleet()


    # RANDOM BOATS
    player_coordinates = player.boats_generator(fleet, ocean)
    pc_coordinates = pc.boats_generator(fleet, ocean)
    ...
    while play:
        ...
        print(f'\n{name_user} estos son tus disparos.', player.shots, f'{name_user} estos son tus barcos.', player.boats, sep = '\n\n')

        if len(player_coordinates) == 0 or len(pc_coordinates) == 0:
            break

        utils.shoot_pc(player_coordinates, pc, player)
        print('PC estos son tus disparos.', pc.shots, 'PC estos son tus barcos.', pc.boats, sep = '\n\n')
